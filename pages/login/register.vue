	...
	<view class="container">
		<view class="wrapper">
			...
			<view class="footer">
				...
				<!-- 协议地址 -->
				<navigator url="/my/seting/mimi" open-type="navigate">《隐私政策》</navigator>
				和
				<navigator url="/my/seting/xieyi" open-type="navigate">《用户服务协议》</navigator>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		onLoad() {
			...
			this.invitation = this.$queue.getData('invitation') ? this.$queue.getData('invitation') : '';
			// #ifdef APP-PLUS
			this.platform = 'app'
			//#endif
			//#ifdef H5
			...
			toLogin() {
				...
				if (!userName) {
					...
					this.$Request.post(`/app/Login/registApp`, {
						...
						password: password,
						phone: phone,
						accountOpenId: this.$queue.getData('openid') ? this.$queue.getData('openid') : '',
						...
						if (res.code === 0) {
							...
							uni.setStorageSync('zhiRate',res.user.zhiRate)
